    ...
    API.fetchWithCache('getLeadInitialData', {}, (initialData) => {
        try {
            ...
            if (user.Role === 'Super Admin') {
                ...
                CRMUtils.populateSelect('assignedBranch', branches, 'BranchName', 'BranchName', 'Select Branch');
                CRMUtils.populateSelect('assignedExec', employees, 'EmployeeID', 'Name', 'Select Executive');
            } else if (user.Role === 'Branch Manager') {
                CRMUtils.populateSelect('assignedExec', employees, 'EmployeeID', 'Name', 'Select Executive');
                const allowed = user.Branches ? user.Branches.split(',').map(b=>b.trim()) : [];
                const select = document.getElementById('assignedBranch');
                if (select) {
                    select.innerHTML = '';
                    allowed.forEach(b => {
            ...
            // Pre-fill user as exec if Reception or Exec
            if (user.Role === 'Sales Executive' || user.Role === 'Reception') {
                const execEl = document.getElementById('assignedExec');
                if (execEl) execEl.innerHTML = `<option value="${user.EmployeeID}">${user.Name}</option>`;
            }
            
            // Populate Datalists
            CRMUtils.populateDatalist('dlTyreSizes', tyreSizes, 'Size');
            CRMUtils.populateDatalist('dlBrands', brands, 'Brand');
            
            const companies = [...new Set(vehicles.map(v => v.Company))];
            CRMUtils.populateDatalistFromArray('dlCompanies', companies);
