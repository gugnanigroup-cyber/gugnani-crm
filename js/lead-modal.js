...
// Helper: blur/unblur lead details modal body when Swal is open on top
function _blurLeadModal(blur) {
    const modalBody = document.querySelector('#leadDetailsModal .modal-body');
    if (modalBody) modalBody.style.filter = blur ? 'blur(4px)' : '';
}

// Swal-based Mark Complete / Mark Lost from inside Lead Details modal
window.lmSwalAction = function(action) {
    if (action === 'completed') {
        _blurLeadModal(true);
        Swal.fire({
            ...
            html: `
                ...
                <div class="mb-3 text-start">
                    ...
                    <textarea id="swalCompRemarks" class="form-control" rows="2"></textarea>
                </div>
            `,
            ...
            preConfirm: () => {
                ...
                return { compDate: date, invoiceNo: document.getElementById('swalCompInvoice').value, remarks: document.getElementById('swalCompRemarks').value };
            }
        }).then(async (result) => {
            _blurLeadModal(false);
            if (result.isConfirmed) {
                try {
                    await API.call('markCompleted', { LeadID: lmCurrentLeadId, CompDate: result.value.compDate, InvoiceNo: result.value.invoiceNo, Remarks: result.value.remarks });
                    Swal.fire({ icon: 'success', title: 'Completed!', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
        ...
        Swal.fire({
            ...
            html: `
                ...
                <div class="mb-3 text-start">
                    <label class="form-label fw-bold">Remarks</label>
                    <textarea id="swalLostRemarks" class="form-control" rows="2"></textarea>
                </div>
            `,
            showCancelButton: true,
            ...
            preConfirm: () => {
                ...
                return { reason: reason, remarks: document.getElementById('swalLostRemarks').value };
            }
        }).then(async (result) => {
            _blurLeadModal(false);
            if (result.isConfirmed) {
                try {
                    await API.call('markLost', { LeadID: lmCurrentLeadId, Reason: result.value.reason, Remarks: result.value.remarks });
                    Swal.fire({ icon: 'success', title: 'Marked Lost', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
                    CRMUtils.refreshPageData();
                } catch(e) { Swal.fire('Error', e.message, 'error'); }
            }
        });
...
// Swal-based Schedule Fitment from inside Lead Details modal
window.lmSwalSchedule = function() {
    const lead = lmCurrentLeadId && window.CRMCachedLeads && window.CRMCachedLeads[lmCurrentLeadId];
    _blurLeadModal(true);
    Swal.fire({
        title: 'Schedule Fitment',
        html: `
        ...
        preConfirm: () => {
            ...
            return { date, time: document.getElementById('swalSchTime').value || '10:00', size: document.getElementById('swalSchSize').value, qty: document.getElementById('swalSchQty').value, reserved: document.getElementById('swalSchReserved').value, remarks: document.getElementById('swalSchRemarks').value };
        }
    }).then(async (result) => {
        _blurLeadModal(false);
        if (result.isConfirmed) {
            try {
                const res = await API.call('scheduleFitment', { LeadID: lmCurrentLeadId, FitmentDate: result.value.date, FitmentTime: result.value.time, TyreSize: result.value.size, Quantity: result.value.qty, ReservedStock: result.value.reserved, Remarks: result.value.remarks });
